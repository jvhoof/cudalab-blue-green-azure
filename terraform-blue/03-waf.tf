...
data "template_file" "ansible_host" {
  ...
  vars {
    name      = "${var.PREFIX}-VM-WAF-${count.index}"
    arguments = "ansible_host=${element(var.waf_ip_addresses, count.index)} ansible_connection=local gather_facts=no waf_license_token=${element(var.WAF_LICENSE_TOKENS, count.index)} "
  }
